...
I think the following result can be proved more or less by the same method as~\cite[Theorem~0.4]{ATW08}.

\begin{proposition}
Let $\br$ be a non-negative smoothness distribution on the hyperplane $H$ and $(\Sigma,\br-\mathbf{1}_H)$ be the deletion along $H$.	 Put $t=|\Sigma_n|$.  Suppose that both $S^{\br-\mathbf{1}_H}(\Sigma)$ and $S^{\br}(\Sigma)$ are free.  Suppose that there are $k$ cones of dimension $n-1$ that are contained in $H$ (we necessarily have $k\le t$).  Then there is a basis $F_1,\ldots F_t$ for $S^{\br-\mathbf{1}_H}(\Sigma)$ as an $R$-module so that $\deg(F_1)\le \deg(F_2)\le \cdots\le \deg(F_t)$, and there are indices $1\le i_1\le \cdots \le i_k\le t$ so that $\{\alpha_HF_j\}_{j\in\{i_1,\ldots,i_k\}}\cup \{F_i\}_{i\in [k]\setminus \{i_1,\ldots,i_k\}}$ is a basis for $S^{\br}(\Sigma)$ as an $R$-module.
\end{proposition}
...
%\section{Localization and splines}
%
%Let $G$ be a graph, $R$ a polynomial ring, $\ell: E\to R_1$ a map associating edges to linear forms, $\br:E\to \ZZ_{\ge -1}$ a smoothness distribution, and $S^r(G,\ell)$ the module of generalized splines associated to this data.
%
%In this section we discuss the behavior of $S^r(G,\ell)$ under localization at a prime ideal $P\subset R$.  Let $G_P=(E_P,V_P)$ be the subgraph of $G$ on the same vertex set (so $V_P=V$), where $E_P=\{e\in E: \ell(e)\in P\}$.  We write $\ell_P, \br_P$ for the restriction of $\ell$ and $\br$ to $E_P$.
%
%\begin{lemma}
%Suppose $G_P$ has connected components $G^1_P,\ldots,G^c_P$.  Then
%\[
%
%\]
%\end{lemma}
